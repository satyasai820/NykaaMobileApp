...
const ProductComponent = () => {
    ...
    return (
        <Imports.ScrollView showsVerticalScrollIndicator={false}>
            <Imports.View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
                <Imports.View style={{ height: 80, }}>
                    <Swiper
                        dot={<Imports.View style={styles.dot} />}
                        activeDot={<Imports.View style={styles.activeDot} />}
                        paginationStyle={styles.pagination}
                        autoplay={true} autoplayTimeout={4}>
                        {bannerImages.map((item, index) => (
                            <Imports.Image key={index} style={{ width: '100%', height: 80 }} source={{ uri: item.img }} />
                        ))}
                    </Swiper>
                </Imports.View>
